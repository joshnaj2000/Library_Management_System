...
from ..handler import sql_handler 
from ...schemas.model import User , Book

app = FastAPI()
router  = APIRouter()
ser = sql_handler.Service()
...
@router.put("/borrow-book/{user_id}")
def borrow_book(user_id:int , book_id:int):
    return ser.borrow_book(user_id , book_id)

@router.put("/return_book/{book_id}")
def return_book(user_id:int , book_id:int):
    return ser.return_book(user_id , book_id)

@router.get("/get-borrowed-books/{user_id}")
def get_borrowed_books(user_id :int):
    return ser.get_borrowed_books(user_id)

@router.get("/get-user-by-start-letter/{fname}")
def get_user_by_letter(letter:str):
    return ser.find_user_first_letter(letter)

@router.get("/get-all-users-by-name/{fname}")
def get_all_users():
    return ser.users()

@router.get("/get-user-by-book/{book_id}")
def get_users_by_book(book_id:int):
    return ser.left_join(book_id)

@router.get("/right-join")
def right_join():
    return ser.right_join()

@router.get("/cross-join") 
def cross_join():
    return ser.cross_join()


sql_service_router = router
...
#                 result = self.db.db.commit()
#                 logging.info("User Inserted with user id : {}".format(user_id))
#                 return result
#             else:
...
#                 self.db.db.commit()
#                 logging.info("Book Inserted with Book id :{}".format(book_id))
